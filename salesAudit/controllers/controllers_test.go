...
// fixture: L1 is overdue with an unverified down payment; L2 is a fresh EMI lead with everything
// verified; L3 is in another stage (not listed, but reachable by id).
func newFixture() *store.Memory {
	s := store.NewMemory()
	s.Leads = []models.ZohoLead{
		...
		{ID: "L3", ZenID: "Z3", Stage: "Zen Student Program", StudentFullName: "Learner Three"},
	}
	s.Payments = []models.ZohoPayment{
		{ID: "P1", AllEnrolment: "Z1", Type: models.CreditBookingAmount, Amount: "999", Verified: "", PaymentDate: "2026-09-20", AddedTime: "20-Sep-2026 10:05:00"},
		{ID: "P2", AllEnrolment: "Z1", Type: models.CreditPart1, Amount: "30000", Verified: "Yes", AddedTime: "21-Sep-2026 10:00:00", VerifiedOn: "2026-09-22T04:00:00Z"},
		{ID: "P3", ZenID: "Z2", Type: models.CreditBookingAmount, Amount: "999", Verified: "Yes", AddedTime: "24-Sep-2026 15:10:00"},
		{ID: "P4", AllEnrolment: "Z9", Type: models.CreditPart1, Amount: "1", AddedTime: "24-Sep-2026 15:10:00"},
	}
	s.Emis = []models.ZohoEmi{{ID: "E1", ZenID: "Z2", EMIVendor: "Loan Partner", LoanAmount: "18000.00", FirstEMI: "1800.00", ROI: "0", TenorInMonths: "10", EMIStatus: "Disbursed"}}
	s.Partials = []models.ZohoPartialReminder{
		{StudentID: "L1", Amount: "22500.00", DueDate: "20-Nov-2026"},
		{StudentID: "L1", Amount: "30000.00", DueDate: "20-Oct-2026"},
	}
	s.Discounts = []models.ZohoDiscount{{LearnerEmail: "ONE@example.com", Status: "Approved", DiscountValue: "5000", AddedTime: "19-Sep-2026 10:00:00"}}
	return s
}

func newRouter(s store.Store) (*gin.Engine, *fakeQueue, *service.Service) {
	gin.SetMode(gin.TestMode)
	queue := &fakeQueue{}
	svc := &service.Service{Store: s, Mail: queue, AccountsEmail: "accounts@example.com", Now: func() time.Time { return now }}
	engine := gin.New()
	routes.Register(engine, controllers.New(svc), program)
	return engine, queue, svc
}
...
func TestRequiresAuthorization(t *testing.T) {
	engine, _, _ := newRouter(newFixture())
	recorder := httptest.NewRecorder()
	engine.ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/sales-audit/leads", nil))
...
func TestGetLeads(t *testing.T) {
	s := newFixture()
	s.AlertDocs = []models.Alert{
		{ID: "a1", Program: program, LeadID: "L1", Kind: models.AlertEscalation, Mail: models.Mail{Trigger: models.TriggerAuto, SentAt: now.Add(-10 * time.Minute).Unix()}},
		{ID: "a2", Program: "other", LeadID: "L1", Kind: models.AlertEscalation, Mail: models.Mail{Trigger: models.TriggerAuto, SentAt: now.Unix()}},
	}
	engine, _, _ := newRouter(s)
...
func TestGetLeadsStoreError(t *testing.T) {
	s := newFixture()
	s.FailNextListAuditLead = errors.New("mongo down")
	engine, _, _ := newRouter(s)
	code, body := call(t, engine, http.MethodGet, "/sales-audit/leads", nil)
	expect(t, code, body, http.StatusInternalServerError)
}

func TestLeadSummaries(t *testing.T) {
	engine, _, _ := newRouter(newFixture())
	code, body := call(t, engine, http.MethodGet, "/sales-audit/leads/summaries", nil)
	expect(t, code, body, http.StatusOK)
...
func TestSendReminder(t *testing.T) {
	...
	if len(queue.alertIDs) != 1 || len(s.AlertDocs) != 1 || s.AlertDocs[0].Created.By != "user-hash" {
		t.Errorf("queued %v, logged %+v", queue.alertIDs, s.AlertDocs)
	}
...
func TestCcResponse(t *testing.T) {
	...
	expect(t, code, body, http.StatusOK)
	if len(s.CcResponseDocs) != 1 || s.CcResponseDocs[0].Response != models.CcMailSentAwaitingAck || s.CcResponseDocs[0].Alert != nil {
		t.Errorf("responses = %+v, want one updated record", s.CcResponseDocs)
	}
}

func TestLeadAudit(t *testing.T) {
	s := newFixture()
	s.RecheckDocs = []models.Recheck{{ID: "r1", Program: program, LeadID: "L1", Status: models.RecheckOpen}}
	engine, _, _ := newRouter(s)
...
func TestLeadAuditUsesCcExtract(t *testing.T) {
	s := newFixture()
	s.CcExtracts = []models.CcExtract{{Program: program, LeadID: "L2", PointsCovered: []string{"fee"},
...
func TestRechecks(t *testing.T) {
	...
	if recheck.Status != models.RecheckOpen || recheck.Alert.Subject != "Recheck raised (Payment): Learner One" ||
		len(recheck.Alert.To) != 2 || len(queue.alertIDs) != 1 {
...
func TestAuditHistory(t *testing.T) {
	s := newFixture()
	s.AlertDocs = []models.Alert{
		{ID: "old", Program: program, LeadID: "L1", Kind: models.AlertEscalation, Mail: models.Mail{SentAt: now.AddDate(0, 0, -90).Unix()}},
		{ID: "new", Program: program, LeadID: "L1", Kind: models.AlertEscalation, Mail: models.Mail{SentAt: now.AddDate(0, 0, -1).Unix()}},
	}
	s.RecheckDocs = []models.Recheck{{ID: "r1", Program: program, LeadID: "L1", RaisedAt: now.AddDate(0, 0, -100).Unix(), Status: models.RecheckOpen}}
	engine, _, _ := newRouter(s)
